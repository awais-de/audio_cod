...
ckpt_path = "checkpoints_emergency/phase1_multiscale_20260129_124452/best.pt"
print(f"📦 Loading Phase 1 checkpoint: {ckpt_path}")

model_p1 = NeuralAudioCodec().to(device)
checkpoint = torch.load(ckpt_path, map_location=device)
model_p1.load_state_dict(checkpoint)  # Direct state_dict
model_p1.eval()
print("✅ Phase 1 model loaded\n")

# Load V3 baseline checkpoint
v3_path = "checkpoints_emergency/pesq_balanced_v3_20260129_094112/best.pt"
print(f"📦 Loading V3 baseline: {v3_path}")
model_v3 = NeuralAudioCodec().to(device)
checkpoint_v3 = torch.load(v3_path, map_location=device)
model_v3.load_state_dict(checkpoint_v3)  # Direct state_dict
model_v3.eval()
...
if not audio_files:
    print("⚠️ No test audio files found. Checking for alternatives...")
    audio_files = list(test_audio_dir.glob("**/*.wav"))[:10]
...
if snr_scores_p1:
    ...
    print(f"   Δ SNR:   {snr_gain:+.3f} dB ({snr_gain/avg_snr_v3*100:+.1f}%)")
    
    print(f"\n✅ Phase 1 evaluation complete")
else:
    print("\n⚠️ No samples evaluated")
