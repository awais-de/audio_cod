#!/usr/bin/env python3
"""
Phase G: Fine-Polish at Ultra-Low LR
======================================
Continues from Phase F checkpoint with:
  - Same CombinedSpectralLoss (lin STFT + log STFT + log mel)
  - Same noise augmentation (60%, 5-20 dB)
  - LR 10× lower than Phase F: 2e-7 (micro adjustments only)
  - Fewer epochs (20 vs 30) to avoid over-fitting

Goal: squeeze out the last few percentage points of quality without
      destabilising the Phase F solution.
...
def train(args):
    print(f"\n{'='*68}")
    print("PHASE G: FINE-POLISH (ultra-low LR)")
    print(f"{'='*68}")
    ...
    print(f"Loaded Phase F: d_model={d_model}, bottleneck_dim={bottleneck_dim}, "
