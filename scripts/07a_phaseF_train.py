...
Combines three complementary loss terms:
  1. Linear STFT magnitude  — spectral accuracy (Phase C strength)
  2. Log STFT magnitude     — quiet component sensitivity (consonants, sibilants)
  3. Log Mel spectrogram    — perceptual frequency weighting (vowel quality, 1-3kHz)

Each term contributes equally (averaged). No architecture changes.
...
class CombinedSpectralLoss(nn.Module):
    """
    Triple loss: linear STFT + log STFT + log mel spectrogram.
...
def train(args):
    ...
    print(f"Loss             : (linear_STFT + log_STFT + log_mel) / 3")
    ...
    print(f"Loaded Phase C: d_model={d_model}, bottleneck_dim={bottleneck_dim}, stride={temporal_stride}\n")
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 5 == 0:
            torch.save({
                ...
                'd_model': d_model, 'n_layers': n_layers, 'n_heads': n_heads,
                'window_size': window_size, 'bottleneck_dim': bottleneck_dim,
                'temporal_stride': temporal_stride, 'phase': 'F',
            }, args.output / f'epoch_{epoch+1:02d}.pt')

    runs_dir = PROJECT_ROOT / 'runs' / 'temporal_phaseF'
