#!/usr/bin/env python3
"""
V3 - BALANCED FINE-TUNING
Balanced Learning Rate (3e-6 - midpoint between V1's 5e-6 and V2's 1e-5)
Goal: Optimize BOTH PESQ and STOI simultaneously
...
def train_epoch(model, train_loader, optimizer, device):
    ...
    for batch_idx, audio in enumerate(pbar):
        ...
        # Balanced loss: both spectral and time domain weighted equally
...
def main():
    ...
    run_name = f"pesq_balanced_v3_{timestamp}"
    
    # Config - BALANCED PARAMETERS
    sr = 16000
    segment_length = 16000
    batch_size = 4
    n_epochs = 20
    lr = 3e-6  # ← BALANCED: Midpoint between V1 (5e-6) and V2 (1e-5)
    
    init_checkpoint = Path("checkpoints_emergency/finetuned/best.pt")  # Start from V1
    ...
    print(f"Config: LR={lr} (Balanced) | Init from V1 | 20 epochs | 1500 files")
    
    # Load model from V1 best
    print(f"\n=== Loading model from V1 ===")
    ...
    if init_checkpoint.exists():
        ...
        print(f"✓ Loaded from: {init_checkpoint}")
    else:
    ...
    # Training loop
    print(f"\n=== V3 BALANCED FINE-TUNING: {n_epochs} epochs (LR={lr}) ===")
    best_loss = float('inf')
    patience = 3
    patience_counter = 0
    ...
    for epoch in range(n_epochs):
        ...
        # Compute metrics every 2 epochs
    ...
    # Summary
    print(f"\n{'='*60}")
    print(f"=== V3 BALANCED FINE-TUNING COMPLETE ===")
    print(f"{'='*60}")
    print(f"Run Name: {run_name}")
    print(f"Config: Balanced LR (3e-6) | Started from V1 | 1500 files")
    ...
    with open(metadata_path, 'w') as f:
        f.write(f"=== V3 BALANCED FINE-TUNING ===\n")
        f.write(f"Run Name: {run_name}\n")
        f.write(f"Start Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"\nCONFIGURATION:\n")
        f.write(f"  Learning Rate: {lr} (BALANCED - midpoint of V1 & V2)\n")
        ...
        f.write(f"\nOBJECTIVE:\n")
        f.write(f"  Optimize BOTH PESQ and STOI\n")
        f.write(f"  Expected: PESQ 3.0-3.1 | STOI 0.95-0.96\n")
