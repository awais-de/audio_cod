#!/usr/bin/env python3
"""
Compare Neural Audio Codec vs FFmpeg encoder.
Metrics: bitrate (entropy-coded), STOI, PESQ, latency.
"""
...
try:
    from evaluate_scipy_based import pesq_scipy, stoi_scipy
...
class FFmpegCodec:
    def __init__(self, codec='aac', bitrate_kbps=64, sample_rate=16000):
        self.codec = codec
...
class NeuralCodecRunner:
    def __init__(self, checkpoint_path, device='cpu', sample_rate=16000):
        self.device = device
        self.sample_rate = sample_rate
    ...
    def encode_decode(self, audio, chunk_seconds=2.0):
        ...
        with torch.no_grad():
            for start in range(0, len(audio), chunk_size):
                ...
                compressed = zlib.compress(latent_np.tobytes(), level=9)
        ...
        duration = len(audio) / self.sample_rate
        bitrate_bps = (encoded_bytes * 8) / max(duration, 1e-6)
        return reconstructed, bitrate_bps, latencies_ms
...
def main():
    parser = argparse.ArgumentParser(description="Compare Neural Codec vs FFmpeg encoder")
    ...
    parser.add_argument('--ffmpeg-bitrate-kbps', type=int, default=64)
    parser.add_argument('--out', type=Path, default=Path('results/compare_ffmpeg_codec.csv'))
    ...
    neural = NeuralCodecRunner(args.checkpoint, device=args.device, sample_rate=args.sample_rate)
    ffmpeg_codec = FFmpegCodec(codec=args.ffmpeg_codec, bitrate_kbps=args.ffmpeg_bitrate_kbps, sample_rate=args.sample_rate)
    ...
    for audio_path in tqdm(files, desc="Evaluating"):
        ...
        results.append({
            ...
            'ffmpeg_bitrate_bps': ffmpeg_bitrate,
            'ffmpeg_latency_ms': ffmpeg_latency_ms,
    ...
    header = [
        'file', 'duration_s',
        'neural_bitrate_bps', 'neural_latency_ms_avg', 'neural_latency_ms_p95', 'neural_pesq', 'neural_stoi',
        'ffmpeg_codec', 'ffmpeg_bitrate_bps', 'ffmpeg_latency_ms', 'ffmpeg_pesq', 'ffmpeg_stoi',
    ]
