#!/usr/bin/env python3
"""
PESQ-focused finetune starting from best_emergency.pt
- Higher STFT weight, lower STOI weight to push PESQ
- Short run (default 10 epochs) with 1s segments
"""
import os
import sys
...
def main():
    ...
    ckpt_path = Path('checkpoints_emergency/best_emergency.pt')
    ...
    best_pesq = 0.0
    best_ckpt_path = Path('checkpoints_emergency/best_pesq_finetune.pt')
    ...
    for epoch in range(1, num_epochs + 1):
        model.train()
        epoch_loss = 0.0
        pbar = tqdm(train_loader, desc=f'PESQ Epoch {epoch}/{num_epochs}')
        optimizer.zero_grad()
