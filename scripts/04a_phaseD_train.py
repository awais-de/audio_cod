...
=============================================
Replaces the Straight-Through Estimator (STE) with a uniform noise proxy.

During training:
    z_noisy = z + U(-0.5, 0.5) * scale   [fully differentiable]

At inference:
    q = round((z - z_min) / scale)        [same 3-bit quantization as before]

The noise distribution is statistically equivalent to quantization error, so
the decoder learns to reconstruct from realistically perturbed latents rather
than clean latents with an approximated gradient. No STE hack needed.

Noise augmentation (60%, 5-20 dB SNR) is preserved from Phase C.

Loads: checkpoints_active/temporal_phaseC/best.pt
Output: checkpoints_active/temporal_phaseD/
"""
...
# Uniform noise proxy  (replaces STE)

def uniform_noise_quantize(z):
    """
    Differentiable proxy for 3-bit uniform quantization.
...
def train(args):
    print(f"\n{'='*68}")
    print("PHASE D: UNIFORM NOISE PROXY (VAE-STYLE QAT)")
    print(f"{'='*68}")
    ...
    print(f"Quantization     : Uniform noise proxy  U(-0.5, 0.5) * scale")
    print(f"{'='*68}\n")
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, x in enumerate(pbar):
            ...
            z = model.encode(x)
            z_noisy = uniform_noise_quantize(z)   # <-- replaces ste_quantize_3bit
    ...
    # Copy history to runs/
    runs_dir = PROJECT_ROOT / 'runs' / 'temporal_phaseD'
    runs_dir.mkdir(parents=True, exist_ok=True)
    with open(runs_dir / 'training_history.json', 'w') as f:
        json.dump(history, f, indent=2)

    print(f"\nPhase D done. Best loss: {best_loss:.6f}")
...
def main():
    ...
    parser.add_argument('--output', type=Path,
                        default=PROJECT_ROOT / 'checkpoints_active/temporal_phaseD')
