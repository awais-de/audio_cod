...
print("📊 Generating 10 synthetic test signals...\n")
...
for idx in range(1, 11):
    try:
        print(f"[{idx}/10] Synthetic signal {idx}...", end=" ")
        
        # Create synthetic waveform (speech-like)
        t = np.linspace(0, 1, 16000, False)
        # Mix of multiple frequencies to simulate speech
        signal = (
            0.3 * np.sin(2 * np.pi * 200 * t) +  # Fundamental
            0.2 * np.sin(2 * np.pi * 400 * t) +  # 2nd harmonic
            0.15 * np.sin(2 * np.pi * 600 * t) +  # 3rd harmonic
            0.1 * np.sin(2 * np.pi * 100 * (1 + 0.5 * np.sin(2 * np.pi * 3 * t)) * t)  # FM component
        )
        signal = np.random.randn(16000) * 0.05 + signal  # Add some noise
        waveform = torch.from_numpy(signal.astype(np.float32)).unsqueeze(0).unsqueeze(0)
...
if snr_scores_p1:
    print(f"\n{'='*70}")
    print(f"PHASE 1 EVALUATION RESULTS (Synthetic Audio)")
    print(f"{'='*70}\n")
    ...
    print(f"\nInterpretation:")
    print(f"  • Positive Δ SNR → Phase 1 reconstruction is CLOSER to input")
    print(f"  • Negative Δ SNR → V3 reconstruction is CLOSER to input")
